...
local address   = read_config("address") or error("ES address should be defined")
local port      = read_config("port") or 9200
local timeout   = read_config("timeout") or 10
local discard   = read_config("discard_on_error") or true

local batch_file        = string.format(
    "%s/%s-output.batch", read_config("output_path"), read_config("Logger") or 'ceilometer-es')
